...
/**
 * GUI class for the professor's exam scheduling window. 
 * 
 * @version 17 Jun 2023
 * @author Michalis Vafeiadis, Dimitra Papadopoulou, Eleni Loula
 */
public class ScheduleExamGUI extends JFrame implements ActionListener, MouseListener {
	...
	public ScheduleExamGUI(Course selectedCourse) {
		...
		ES = ExamScheduler.getInstance();//access ES
	...
	/**
	 * Button listener for search and confirm button
	 * If the user selects the search button, the hoursbox gets loaded with all currently available hours in the selected date(created 
	 * at that time, if not already exists). The confirm button registers the selected inputs for the class and updates the exam scheduler 
	 * lists and the availability.
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == searchButton) {
			...
			for(i=0; i<coursesArray.length; i++) {
				if(!(coursesArray[i] instanceof Course)) {//if there is not an already inserted course object at that index
	...
	/**
	 * Mouse listener for dynamically selecting rooms and updating the remaining students
	 * 
	 * @param e
	 */
	...
	/**
	 * Hash map for direct relation between room codenames and room objects
	 * @return
	 */
	...
	/**
	 * Subclass for extending the functionality of the JList, allowing for multiple selection without pressing ctrl key
